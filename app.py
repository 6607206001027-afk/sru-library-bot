...
def fetch_all_books():
    """
    ดึงหนังสือทั้งหมดจาก Firestore เพียงครั้งเดียวต่อข้อความ แล้วส่งต่อให้ทั้งฟังก์ชันดึงบริบทหนังสือ
...
def get_books_context(all_docs, user_msg="", limit=15):
    """
    หยิบตัวอย่างหนังสือจากฐานข้อมูลมาให้ Gemini อ่านประกอบการตอบ
    รับ all_docs ที่ดึงมาแล้วจาก fetch_all_books() เพื่อไม่ต้องอ่าน Firestore ซ้ำ
    (พารามิเตอร์ user_msg รับไว้เผื่ออนาคต ตอนนี้ยังไม่ได้ใช้กรองผลลัพธ์)
    """
    if not all_docs:
...
def handle_message(event):
    ...
    13. หนังสือใน [ตัวอย่างหนังสือจากฐานข้อมูล] ด้านล่างนี้ เป็นตัวอย่างบางส่วนจากฐานข้อมูลทั้งหมด ไม่ใช่ผลการค้นหาที่กรองตรงกับคำถามผู้ใช้เป๊ะๆ ถ้าไม่เจอเล่มที่ตรงกับที่ผู้ใช้ถามในตัวอย่างนี้ ให้บอกตามตรงว่าไม่พบในตัวอย่างที่มี และแนะนำให้ลองถามชื่อ/หัวข้ออื่น หรือใช้เมนู OPAC ค้นหาโดยตรง ห้ามหยิบหนังสือที่ไม่เกี่ยวข้องมาแนะนำแทนโดยไม่บอกผู้ใช้ว่าไม่ตรงกับที่ถาม

    [ตัวอย่างหนังสือจากฐานข้อมูล]
    {books_context}
